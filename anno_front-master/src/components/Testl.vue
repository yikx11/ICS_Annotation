...
export default class Testl extends Vue {
    ...
    clearCanvas() {
        ...
        this.bgeraser.height = this.bg.height
    ...
    operate(key: string) {
        ...
        switch (key) {
            ...
            case 'b':   // 'b' --> 'bbox'
                // 添加多边形标注
                this.bg.onpointerdown = this.Yonpointerdown
                this.bg.onpointerout = this.Yonpointerout
                this.bg.onpointerup = this.Yonpointerup
                this.bg.onpointermove = this.Yonpointermove
                // 返回原来
            ...
            case 'B':   // 'B' --> 'Brush'
                ...
                // this.bgeraser.onpointerdown = this.brushonpointerdown
                // this.bgeraser.onpointerout = this.brushionpointerout
                // this.bgeraser.onpointerup = this.brushonpointerup
                // this.bgeraser.onpointermove = this.brushonpointermove
                this.bg.onmousemove = this.YmouseMove
                this.bg.onmousedown = this.YmouseDown
                this.bg.onmouseup = this.YmouseUp
                this.bg.onpointerdown = this.brushonpointerdown
                this.bg.onpointerout = this.brushionpointerout
                this.bg.onpointerup = this.brushonpointerup
                this.bg.onpointermove = this.brushonpointermove
                break
            default:
        }
        ...
        switch (key) {
            ...
            case 's':
                // 保存标注编辑
                if (this.brush_mode) {
                    let image = this.bg.toDataURL("image/png")
    ...
    brushdraw(mousex: any, mousey: any, ctrlKey: any) {
        ...
        if (this.brush_points.length == 2) {
            this.context.moveTo(this.brush_points[this.brush_points.length - 2].x, this.brush_points[this.brush_points.length - 2].y)
            this.context.lineTo(x, y)
        } else {
            ...
            this.context.quadraticCurveTo(this.brush_points[this.brush_points.length - 2].x, this.brush_points[this.brush_points.length - 2].y, x, y)
        }
        this.context.stroke()
    ...
    mounted() {
        ...
        // this.contextbrush = this.bg.getContext('2d') as CanvasRenderingContext2D
        ...
        /* ---------------------------------------------------- */
        this.Yonpointerup = this.bg.onpointerup
        this.Yonpointerdown = this.bg.onpointerdown
        this.Yonpointermove = this.bg.onpointermove
        this.Yonpointerout = this.bg.onpointerout
        this.YmouseMove = this.bg.onmousemove
        this.YmouseDown = this.bg.onmousedown
        this.YmouseUp = this.bg.onmouseup
        // this.bg.onmousemove = this.mouseMove
        // this.bg.onmousedown = this.mouseDown
        // this.bg.onmouseup = this.mouseUp
        // collection selecter
