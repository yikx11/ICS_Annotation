...
class DLA(nn.Module):
    ...
    def load_pretrained_model(self, pretrained, name):
        if pretrained:
            data = namedtuple(
                'Dataset', [
                    'model_hash', 'classes', 'mean', 'std',
                    'eigval', 'eigvec', 'name'
                ]
            )(
                name='imagenet',
...
def get_base(base_name, pretrained=False):
    name2encoder = {
        'dla34': dla34,
        'dla60': dla60,
        'dla102': dla102,
    ...
    return name2encoder[base_name](pretrained, return_levels=True)
