...
class UNetBody(nn.Module):
    ...
    def __init__(self, num_classes, base_n_filter=8):
...
class ResidualUNet(nn.Module):
    ...
    def __init__(self, num_classes, infer_size, **kwargs):
        super(ResidualUNet, self).__init__()
        self.infer_size = infer_size
        self.unet_body = UNetBody(num_classes=num_classes, **kwargs)

    def forward(self, inputs):
        '''
        Parameters:
            inputs: torch.FloatTensor(b, 1, d, h, w)
        Return:
            all_seg_res: torch.LongTensor(b, d, h, w)
        '''
        if self.infer_size is None:
            infer_size = inputs.size(2)
        else:
            infer_size = self.infer_size
        infer_stride = infer_size // 2
        batch_imgs = inputs
        b, c, d, h, w = batch_imgs.size()
        all_seg_score = torch.zeros(b, d, h, w).to(device=inputs.device)
        all_seg_res = torch.zeros(b, d, h, w).to(device=inputs.device).long()
        for d_off in range(0, d - infer_size + infer_stride, infer_stride):
            if (d_off + infer_size) > d:
                d_off = d - infer_size
            imgs = batch_imgs[:, :, d_off : d_off + infer_size]
            out = self.unet_body(imgs)
            out = out.softmax(dim=1)
            seg_score, seg_res = torch.max(out, dim=1)
            local_seg_res = all_seg_res[:, d_off : d_off + infer_size]
            local_seg_score = all_seg_score[:, d_off : d_off + infer_size]
            deassign_index = seg_score > local_seg_score
            local_seg_res[deassign_index] = seg_res[deassign_index]
            local_seg_score[deassign_index] = seg_score[deassign_index]
        return all_seg_score, all_seg_res


if __name__ == '__main__':
    model = ResidualUNet(num_classes=9, infer_size=16)#.cuda()
    image = torch.zeros(1, 1, 32, 32, 32)#.cuda(non_blocking=True)
    target = torch.ones(1, 32, 32, 32)
    with torch.no_grad():
        output = model(image)
    print(output[1].size())
